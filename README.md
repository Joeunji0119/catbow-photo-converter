...
| **`Styled components`** | css를 파일 분리 없이 유지 보수 할 수 있고 조건부 스타일링을 구현하는데 편리해 사용을 결정하였습니다. props나 state에 따른 동적 스타일링이 가능해 button 컴포넌트에 유용하게 사용하였고 자주 쓰이는 색과 css를 ThemeProvider로 전역 관리해 재사용하였습니다. |
